...
<tr><td colspan="3"><h3><?php if($item['present'] =='1'){ ?>Arrived <?php }else{?>Signed Out<?php } ?></h3></td></tr>   
   <?php 
}
$present = $item['present'];

?>
<tr class="item " data-id="<?php echo $item['outputid']; ?>" >
<td >
<h3 data-cat="title" ><?php echo $item['name']; ?></h3>
...
if($_POST['action'] =='Teacher' || $teacher){
  ...
  $items=  mysql_query("SELECT *, CONCAT(firstname, ' ', lastname) as name, DATE_FORMAT(expectedtimestamp ,'%T') as expected, DATE_FORMAT(bookings.actualtimestamp ,'%T') as actualtime, students.id as tblstudentsid, bookings.id as bookingid FROM bcs_students as students RIGHT JOIN bcs_bookings as bookings ON students.id = bookings.studentid  LEFT JOIN (SELECT *, bcs_present.id as presentid, DATE_FORMAT(intime, '%T') as timein FROM bcs_present WHERE DATE_FORMAT(intime, '%d/%m/%Y') = '$date') as present ON present.studentid = students.id  WHERE students.form LIKE '$year%' AND DATE_FORMAT(bookings.expectedtimestamp,'%d/%m/%Y') = '$date' AND bookings.staff = '$username' ORDER BY bookings.seen, lastname, firstname")or die(mysql_error());
	...
	$seen =$item['seen']
	    ...
	    ?>
	    <tr data-id="<?php echo $item['tblstudentsid']; ?>" data-bookingid="<?php echo $item['bookingid']; ?>" class="item">
		<td><?php echo $item['name'];
		if( !is_null($item['timein'])){
		 ?>
		 <div class="small">In: <?php echo $item['timein']; ?>
		 <?php if($item['seen']=='1' ){ ?>
		 Appointment: <?php echo $item['expected']; ?>
		 <?php } ?>
		...
		<?php if($item['seen']=='1' ){ echo $item['actualtime']; }
		else{
		    echo $item['expected'];
		}
		?>
		...
		if(is_null($item['timein']) || $item['seen']=='0'){
		    ...
		    ?>
		   <button data-action="seestudent" data-present="1" class="green"><?php if(is_null($item['timein'])){ echo "Arrive & ";} ?>See</button>
		    <?php
			}
		}
...
if($item['form'] != $form){
    ?>
    <tr class="<?php if(!is_null($form)){ echo "break";} ?>"><td colspan=5></td></tr>
<tr><td colspan=5 ><h3><?php echo $item['form']; ?></h3></td></tr>    
    <?php 
}
$form = $item['form'];
?>
...
<td><?php echo $item['intime']; ?></td>
<td><?php echo $item['peopleseen']; ?></td>
<td style="width:50px;">
<?php
if( is_null($item['present']) ){
?>
...
while($item = mysql_fetch_array($noshow)){
    ?>
    <tr class="item"><td><?php echo $item['name'] ?></td><td><?php echo $item['form'] ?></td><td colspan=3></td></tr>    
    <?php
}
